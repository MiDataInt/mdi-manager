...
version3ToIntegers <- function(version3){ # version3 is major.minor.patch
    x <- as.integer(strsplit(version3, "\\.")[[1]])
    names(x) <- c('major', 'minor', 'patch')
    as.list(x) # i.e. integer(major, minor, patch)
}
version3ToSortableInteger <- Vectorize(function(version3){ # version3 is major.minor.patch
    x <- as.list(as.integer(strsplit(version3, "\\.")[[1]]))
    names(x) <- c('major', 'minor', 'patch')    
